...
// Set up the page.
$indexurl = new moodle_url('/theme/uofr_conservatory/darkmode.php', array('courseid' => $courseid));
$PAGE->set_url($indexurl);
$PAGE->set_pagelayout('report');
$PAGE->set_title(get_string('darkmodepref', 'theme_uofr_conservatory'));
...
if (!empty($action)) {
    ...
    switch ($action) {
        case 'auto' :		
			echo $OUTPUT->header();
            echo $OUTPUT->notification(get_string('darkmodeautosuccess', 'theme_uofr_conservatory'), 'notifysuccess');
            break;
        case 'enable' :			
            echo $OUTPUT->header();
            echo $OUTPUT->notification(get_string('darkmodeenablesuccess', 'theme_uofr_conservatory'), 'notifysuccess');
            break;
        case 'disable' :			
            ...
            //}
			echo $OUTPUT->header();
            echo $OUTPUT->notification(get_string('darkmodedisablesuccess', 'theme_uofr_conservatory'), 'notifysuccess');
...
$renderer = $PAGE->get_renderer('theme_uofr_conservatory', 'darkmodeprefs');
...
$darkmodecheck = ($DB->get_record('theme_conservatory_darkmode', array('userid'=>$USER->id, 'darkmode'=>1))) ? 'yep' : 'nope';
