...
import $ from 'jquery';
import * as Repository from 'theme_urcourses_default/block_myoverview_repository';
...
const TEMPLATES = {
    COURSES_CARDS: 'theme_urcourses_default/block_myoverview/view-cards',
    COURSES_LIST: 'theme_urcourses_default/block_myoverview/view-list',
    COURSES_SUMMARY: 'theme_urcourses_default/block_myoverview/view-summary',
    NOCOURSES: 'core_course/no-courses',
    SUMMARYMODAL: 'theme_urcourses_default/block_myoverview/course-summary-modal-body'
};
...
const showCourseSummary = async (courseId) => {
    const modalContext = await Repository.getCourseSummary({courseid: courseId});
    Modal.create({
        title: modalContext.fullname,
        body: Templates.render(TEMPLATES.SUMMARYMODAL, modalContext),
        removeOnClose: true,
        show: true,
        large: true
    });
...
const registerEventListeners = (root, page) => {
    ...
    root.on(CustomEvents.events.activate, SELECTORS.ACTION_SHOW_COURSE_SUMMARY, (e, data) => {
        const target = $(e.target).closest(SELECTORS.ACTION_SHOW_COURSE_SUMMARY);
        const courseId = getCourseId(target);
        showCourseSummary(courseId);
        data.originalEvent.preventDefault();
    });
