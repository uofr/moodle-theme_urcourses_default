 ...
 * @return string
 */
function theme_uofr_conservatory_get_extra_scss($theme) {
 ...
 */
function theme_uofr_conservatory_get_precompiled_css() {
    global $CFG;
...
function theme_uofr_conservatory_extend_navigation_user_settings($navigation, $user, $usercontext, $course, $coursecontext) {
    ...
    $ondarkmodepage = $PAGE->url->compare(new moodle_url('/theme/uofr_conservatory/darkmode.php'), URL_MATCH_BASE);
	...
	//if (get_config('theme_boost_union', 'enabledarkmode')) {
        $url = new moodle_url('/theme/uofr_conservatory/darkmode.php');
        $darkmodenode = navigation_node::create(get_string('darkmodepref', 'theme_uofr_conservatory'), $url,
