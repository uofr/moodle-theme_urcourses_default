...
class course_summary_exporter extends \core\external\exporter {
    ...
    public function __construct($data, $related = array()) {
        ...
        if (!array_key_exists('hasnotifications', $related)) {
            $related['hasnotifications'] = false;
        }
        if (!array_key_exists('mailcount', $related)) {
            $related['mailcount'] = 0;
        }
        if (!array_key_exists('maillink', $related)) {
            $related['maillink'] = '';
        }
        if (!array_key_exists('faculty', $related)) {
            $related['faculty'] = '';
        }
        if (!array_key_exists('ishidden', $related)) {
            $related['ishidden'] = false;
        }
        if (!array_key_exists('studentmsg', $related)) {
            $related['studentmsg'] = '';
        }
        if (!array_key_exists('studentstate', $related)) {
            $related['studentstate'] = '';
        }
    ...
    protected static function define_related() {
        ...
        return array('context' => '\\context', 'isfavourite' => 'bool?', 'hasnotifications' => 'bool?', 'mailcount' => 'int?', 'maillink' => 'string?', 'faculty' => 'string?', 'ishidden' => 'bool?', 'studentmsg' => 'string?', 'studentstate' => 'string?');
    }
    ...
    protected function get_other_values(renderer_base $output) {
		...
		$studentmsg = 'This course is currently unavailable to students';
		
		if ($this->data->enddate > 0 && time() > $this->data->enddate) {
			$studentstate = 'info';
			$studentmsg = 'This course ended '.userdate($this->data->enddate, '%B %d, %Y');
		} else if ($this->data->enddate > 0 && time() < $this->data->enddate && $this->data->startdate > 0 && time() > $this->data->startdate) {
			$studentstate = 'warning';
			$studentmsg = 'This course is currently unavailable to students';
		} else if ($this->data->startdate > 0 && time() < $this->data->startdate) {
			$studentstate = 'success';
			$studentmsg = 'This course begins '.userdate($this->data->startdate, '%B %d, %Y');
		}
		
		$viewurl = (new moodle_url('/course/view.php', array('id' => $this->data->id)))->out(false);
		
		$coursecontext = \context_course::instance($this->data->id);
		
		if (has_capability('moodle/course:viewhiddensections', $coursecontext)) {
			$infourl = $viewurl;
		} else {
			$infourl = $CFG->wwwroot.'/?redirect=0#summary-'.$this->data->id;
		}
        ...
        return array(
            ...
            'hasnotifications' => $this->related['hasnotifications'],
            'mailcount' => $this->related['mailcount'],
            'maillink' => $this->related['maillink'],
            'faculty' => $this->related['faculty'],
            'ishidden' => $this->data->visible,//$this->related['ishidden'],
    ...
    public static function coursecolor($courseid) {
		...
		/* course color needs changing - URTODO
        // The colour palette is hardcoded for now. It would make sense to combine it with theme settings.
        //$basecolors = ['#81ecec', '#74b9ff', '#a29bfe', '#dfe6e9', '#00b894',
        //    '#0984e3', '#b2bec3', '#fdcb6e', '#fd79a8', '#6c5ce7'];
		
		//$basecolors = ['#ffc82e','#ffc82e','#ffc82e','#ffc82e','#ffc82e','#ffc82e','#ffc82e','#ffc82e','#ffc82e','#ffc82e'];
		//$basecolors = ['#004f2e','#004f2e','#004f2e','#004f2e','#004f2e','#004f2e','#004f2e','#004f2e','#004f2e','#004f2e'];
		$basecolors = ['#cccccc','#cccccc','#cccccc','#cccccc','#cccccc','#cccccc','#cccccc','#cccccc','#cccccc','#cccccc'];
